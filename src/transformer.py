...
def tokenize_pairs(t_kmer, i_kmer):
    """Returns pair of tokenized kmers"""
...
def dataset_from_fasta(fasta_filepath, train=0.7, val=0.2, test=0.1):
    ...
    i_kmers = []
    t_kmers = []
    asddf = list(range(1000))
    nt = int(train*len(gene_seqs.keys()))
    nv = int((train+val)*len(gene_seqs.keys()))
    # ns = int(test*len(gene_seqs.keys()))
    print(asddf[:nt])
    print(len(asddf[:nt]))
    print(asddf[nt:nv])
    print(len(asddf[nt:nv]))
    print(asddf[nv:])
    print(len(asddf[nv:]))
    # train_labels = gene_seqs.keys()[:train*len(gene_seqs)]
    # val_labels = 
    # test_labels = 
    # def create_dataset_subset(seq_labels, gene_seqs):
    #     for seq_label in seq_labels:
    #         seq = gene_seqs[seq_label][:]
    #         kmers = build_kmers(seq, 11)
    #         i_kmers.extend(kmers[:-1])
    #         t_kmers.extend(kmers[1:])
...
dataset = dataset_from_fasta("data/dataset_1000.fasta")
